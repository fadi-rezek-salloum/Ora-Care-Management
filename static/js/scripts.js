...
isLeapYear = (year) => {
    return (year % 4 === 0 && year % 100 !== 0 && year % 400 !== 0) || (year % 100 === 0 && year % 400 ===
        0)
}
...
generateCalendar = (month, year) => {
    ...
    for (let i = 0; i <= days_of_month[month] + first_day.getDay() - 1; i++) {
        ...
        day.addEventListener('click', () => {
            window.location.href = `/${year}/${month+1}/${i - first_day.getDay() + 1}/`
        })
    }
}
...
let date = url.match(/\/([1-2][09][0-9]{2})\/([0-1]?[0-9])\//);
